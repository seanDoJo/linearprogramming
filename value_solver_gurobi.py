...
#        status = lp.solve(GLPK(msg=0))
    ...
    for (e, t) in edgeArray:
        if e.getAttr(GRB.Attr.X):
            frm = var_mapping[e.getAttr(GRB.Attr.VarName)].split(",")[0].strip()
            to = var_mapping[e.getAttr(GRB.Attr.VarName)].split(",")[1].strip()
    ...
    if len(chosenEdges) > 0:
        ...
        while last != "HOME":
            ...
            for (y, k, p) in timeArray:
                if var_mapping[y.getAttr(GRB.Attr.VarName)] == last:
                    last_t = y
                    key = k
                    break
            last_d = None
            for (y, k, p) in decisionArray:
                if var_mapping[y.getAttr(GRB.Attr.VarName)] == last:
                    last_d = y
                    break
            print("{}: time: {}, place decision: {}".format(last, last_t.getAttr(GRB.Attr.X), last_d.getAttr(GRB.Attr.X)))
            pla = None
            for place in data["place_data"][key]:
                if place["name"] == last:
                    pla = place
                    break
            time_s = last_t.getAttr(GRB.Attr.X)
...
#            var_mapping[tVar.name] = place["name"]
            var_mapping[tVar.getAttr(GRB.Attr.VarName)] = place["name"]
            curr_int += 1
...
#            var_mapping[dVar.name] = place["name"]
            var_mapping[dVar.getAttr(GRB.Attr.VarName)] = place["name"]
            curr_int += 1
...
#        var_mapping[dVar.name] = "{}, {}".format(frm, to)
        var_mapping[dVar.getAttr(GRB.Attr.VarName)] = "{}, {}".format(frm, to)
        curr_int += 1
...
def addObjectiveFunction(data, timeArray, decisionArray, edgeArray, lp):
    ...
    for (x, k, p) in timeArray:
        for keyword in data["place_data"]:
            for place in data["place_data"][keyword]:
                if (var_mapping[x.getAttr(GRB.Attr.VarName)] == place["name"]):
...
#    lp += (sum(map(lambda x: x[0]*(x[2]-budget), decisionArray)) <= 0)
    lp.addConstr(sum(map(lambda x: x[0]*(x[2]-budget), decisionArray)) <= 0, "budget")
...
#        nodeName = var_mapping[x.name]
        nodeName = var_mapping[x.getAttr(GRB.Attr.VarName)]
        inBound = []
        outBound = []

        for (y, t) in edgeArray:
#            frm, to = var_mapping[y.name].split(',')
            frm, to = var_mapping[y.getAttr(GRB.Attr.VarName)]
        ...
        # inbound constraint
#        lp += (sum(inBound) - x == 0)
        lp.addConstr(sum(inBound) - x == 0, "path_inbound")
        
        # outbound constraint
#        lp += (sum(outBound) - x == 0)
        lp.addConstr(sum(outBound) - x == 0, "path_outbound")
...
def addDecisionConstraints(data, timeArray, decisionArray, lp):
    ...
    for (x, k, p) in timeArray:
        for (y, l, q) in decisionArray:
            if (var_mapping[x.getAttr(GRB.Attr.VarName)] == var_mapping[y.getAttr(GRB.Attr.VarName)]):
                tuples.append((x, y, k))
                break
    curr_int = 1
    for (x, y, k) in tuples: 
...
#        c2 = x - y*x.upBound <= 0
#        lp += c2
        lp.addConstr(x - y*x.getAttr(GRB.Attr.UB) <= 0, "cd{}".format(curr_int))
        curr_int += 1

def collectSubtours(edgeArray):
    tours = []
#    edgeCopy = [ k for k in edgeArray if value(k[0])] 
    edgeCopy = [ k for k in edgeArray if k[0].getAttr(GRB.Attr.X)]
   
    for edge in edgeCopy:

        original = var_mapping[edge[0].getAttr(GRB.Attr.VarName)].split(",")[0].strip()

        last = var_mapping[edge[0].getAttr(GRB.Attr.VarName)].split(",")[1].strip()
        ...
        while last != original:
            seen = False
            for e in edgeCopy:
                frm = var_mapping[e[0].getAttr(GRB.Attr.VarName)].split(",")[0].strip()
                to = var_mapping[e[0].getAttr(GRB.Attr.VarName)].split(",")[1].strip()
